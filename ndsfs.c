/**
 * ndsfs.c NDS Rom File system mounter (v1.0)
 * Using FUSE, a NDS rom can be mounted as a file system
 *
 * by Andrew Brampton (me@bramp.net)
...
const char *argp_program_version = "ndsfs 1.0";
const char *argp_program_bug_address = "Andrew Brampton <me@bramp.net>";
...
static int ndsfs_get_size(const uint16_t file_id, uint32_t *start, uint32_t *end) {
    ...
    fseek(rom_fp, rom_header.fat_offset + 8*file_id, SEEK_SET);
	...
	while ( next_path != NULL ) {
		...
		for (current_id=top_file_id; ; current_id++) {
			...
			if ( name_length != path_len ) {
				if ( FAT_IS_DIR(*flags) )
					fseek(rom_fp, name_length + 2, SEEK_CUR);
				else
					fseek(rom_fp, name_length, SEEK_CUR);
				continue;
			}
...
static int ndsfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	if (offset < len) {
		if (offset + size > len)
			size = len - offset;
		fseek(rom_fp, start + offset, SEEK_SET);
		size = fread(buf, 1, size, rom_fp);
	} else
		size = 0;
